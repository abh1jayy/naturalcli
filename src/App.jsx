    ...
    <div className={`min-h-screen transition-colors duration-500 ${isDark ? 'bg-[#050816] text-slate-100' : 'bg-[#f5f7ff] text-slate-900'}`}>
      ...
      <div className="w-[96%] mx-auto flex min-h-screen flex-col px-6 lg:px-10 py-4">
        <motion.header
          ...
          className={`rounded-[24px] border px-4 py-3 backdrop-blur-xl ${isDark ? 'border-white/10 bg-slate-950/55 shadow-[0_20px_80px_rgba(3,10,20,0.45)]' : 'border-slate-200 bg-white/70 shadow-[0_20px_80px_rgba(15,23,42,0.08)]'}`}
        >
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>AI terminal</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`rounded-full border p-2.5 transition hover:-translate-y-0.5 ${isDark ? 'border-white/10 bg-white/10' : 'border-slate-300 bg-white'}`}
              >
                {isDark ? <Sun size={16} /> : <MoonStar size={16} />}
              </button>
              <button
                onClick={startNewChat}
                className={`flex items-center gap-2 rounded-full border px-3 py-2 text-sm transition hover:-translate-y-0.5 ${isDark ? 'border-white/10 bg-white/10' : 'border-slate-300 bg-white'}`}
              >
          ...
          {settingsOpen && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className={`mt-3 flex flex-wrap items-center gap-3 rounded-[20px] border px-3 py-2 text-sm ${isDark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-slate-50'}`}
            >
              <button
                onClick={() => setAutoScroll((prev) => !prev)}
                className={`rounded-full border px-3 py-1.5 transition ${autoScroll ? 'border-cyan-400/30 bg-cyan-400/10 text-cyan-300' : 'border-white/10 bg-white/5 text-slate-400'}`}
              >
        ...
        <div className="mt-4 flex flex-1 gap-6">
          ...
          >
            <div className="flex h-full flex-col">
              <div className={`flex-1 overflow-y-auto rounded-[24px] border p-4 sm:p-6 ${isDark ? 'border-white/10 bg-[linear-gradient(180deg,rgba(15,23,42,0.72),rgba(2,6,23,0.86))]' : 'border-slate-200 bg-white/80'}`}>
                <div className="mx-auto flex h-full w-full flex-col gap-4">
                  ...
                  {isEmptyState && (
                    ...
                    >
                      <div className={`rounded-[22px] border p-4 ${isDark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-slate-50'}`}>
                        ...
                        <div className="grid gap-2">
                          {starterCards.map((card) => (
                            <button
                              ...
                              className={`rounded-2xl border px-3 py-2.5 text-left text-sm transition hover:-translate-y-0.5 ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-white'}`}
                            >
                              {card.title}
                            </button>
                          ))}
              ...
              <div className={`sticky bottom-0 mt-3 rounded-[24px] border p-3 ${isDark ? 'border-white/10 bg-slate-950/70 shadow-[0_12px_40px_rgba(2,6,23,0.28)]' : 'border-slate-200 bg-white/80 shadow-[0_12px_40px_rgba(15,23,42,0.06)]'}`}>
                  ...
                  {quickActions.map((action) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2 rounded-full border px-3 py-2 text-sm transition hover:-translate-y-0.5 ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'}`}
                      >
                ...
                {selectedFileName && (
                  <div className={`mb-2 inline-flex items-center rounded-full border px-2.5 py-1 text-xs ${isDark ? 'border-cyan-400/20 bg-cyan-400/10 text-cyan-300' : 'border-cyan-400/20 bg-cyan-50 text-cyan-700'}`}>
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center gap-2 rounded-2xl border px-3 py-2.5 text-sm transition hover:-translate-y-0.5 ${isDark ? 'border-white/10 bg-white/10' : 'border-slate-300 bg-white'}`}
                  >
                    <Paperclip size={16} /> Upload
                  </button>

                  <div className={`flex flex-1 items-center gap-2 rounded-2xl border px-3 py-2.5 ${isDark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-slate-50'}`}>
                    <input
                      value={input}
                ...
                <div className="flex w-full flex-col gap-3 sticky top-24 max-h-[calc(100vh-6rem)] overflow-auto">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className={`text-sm ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Developer context</p>
                    </div>
                    <button
                      onClick={() => setSidebarOpen(false)}
                      className={`rounded-full border p-2 ${isDark ? 'border-white/10 bg-white/10' : 'border-slate-300 bg-white'}`}
                    >
                      <PanelRight size={14} />
                    </button>
                  </div>

                  <div className={`rounded-[20px] border p-3 ${isDark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-slate-50'}`}>
                  ...
                  <div className={`rounded-[20px] border p-3 ${isDark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-slate-50'}`}>
                    ...
                    <div className="space-y-2 text-sm text-slate-400">
                      {pinnedResponses.length ? pinnedResponses.map((item) => <div key={item} className="rounded-xl border border-white/10 bg-white/5 p-2 text-xs">{item.slice(0, 120)}{item.length > 120 ? '…' : ''}</div>) : <div>No pinned responses yet.</div>}
                    </div>
                  </div>
                </div>
              </motion.aside>
